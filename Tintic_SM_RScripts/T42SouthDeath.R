...
setwd("G:/Shared drives/UGS_Groundwater/Projects/WRI/Tintic/Soil Moisture/T42 South Death Creek/RawData")
...
ggplot(SM_df_tidy2, aes(DateTime, VSM)) +
  ...
  labs(title = "T42 South Death Creek", x = "Date", y = "Soil Moisture m3/m3 VWC") +
  ylim(0, 0.6) +
  scale_color_discrete(breaks = c("Port_1", "Port_2", "Port_3"), labels = c("42 in", "25 in", "12 in"), name = "Sensor Depth")
...
write.csv(Daily_VSM, "G:/Shared drives/UGS_Groundwater/Projects/WRI/Tintic/Soil Moisture/RExports//T42SouthDeathDailyAveVSM.csv")

##monthly mean (not working - averaging all years/month)
...
ggplot(Monthly_Mean, aes(Month_Yr, mean_VSM)) +
  ...
  labs(title = "T42 South Death Creek Monthly Average", x = "Month", y = "Average VSM (m3/m3)") +
