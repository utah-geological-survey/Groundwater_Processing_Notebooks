...
setwd("G:/Shared drives/UGS_Groundwater/Projects/WRI/Tintic/Soil Moisture/T2 Mud 1/RawData")
...
ggplot(SM_df_tidy2, aes(DateTime, VSM)) +
  ...
  labs(title = "T2 Mud 1", x = "Date", y = "Soil Moisture m3/m3 VWC") +
  ylim(0, 0.6) +
  scale_color_discrete(breaks = c("Port_1", "Port_2"), labels = c("4 ft", "1 ft"), name = "Sensor Depth")
  

#Other statistics
#Daily mean (and export for use with precipitation data)
...
write.csv(Daily_VSM, "G:/Shared drives/UGS_Groundwater/Projects/WRI/Tintic/Soil Moisture/RExports//T2Mud1DailyAveVSM.csv")
...
ggplot(Monthly_Mean, aes(Month_Yr, mean_VSM)) +
  ...
  labs(title = "T2 Mud 1 Monthly Average", x = "Month", y = "Average VSM") +
