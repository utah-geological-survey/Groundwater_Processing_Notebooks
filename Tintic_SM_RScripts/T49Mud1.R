...
setwd("G:/Shared drives/UGS_Groundwater/Projects/WRI/Tintic/Soil Moisture/T49 Mud 1 upland/RawData")
...
ggplot(SM_df_tidy2, aes(DateTime, VSM)) +
  ...
  labs(title = "T49 Mud 1", x = "Date", y = "Soil Moisture m3/m3 VWC") +
  ylim(0, 0.6) +
  scale_color_discrete(breaks = c("Port_1", "Port_2", "Port_3"), labels = c("15 ft", "5 ft", "10 ft"), name = "Sensor Depth")
...
write.csv(Daily_VSM, "G:/Shared drives/UGS_Groundwater/Projects/WRI/Tintic/Soil Moisture/RExports//T49Mud1DailyAveVSM.csv")
...
ggplot(Monthly_Mean, aes(Month_Yr, mean_VSM)) +
  ...
  labs(title = "T49 Mud 1", x = "Month", y = "Average VSM (m3/m3)") +
  scale_color_discrete(breaks = c("Port_1", "Port_2", "Port_3")) +
  theme(axis.text.x = element_text(angle=45))

##Monthly mean with plot with error bars
ggplot(Monthly_Mean, aes(Month_Yr, mean_VSM)) +
  geom_point(aes(color = Depth)) +
