...
VSM <- read.csv("G:/Shared drives/UGS_Groundwater/Projects/WRI/Tintic/Soil Moisture/RExports//T47Mud2DailyAveVSM.csv")
...
ggplot(daily, aes(x = Day)) +
  ...
  labs(title = "T47 Mud 2", x = "Date", y = "Average Soil Moisture m3/m3 VWC") +
  scale_color_discrete(breaks = c("Port_1", "Port_2", "Precipitation"), 
                       labels = c("4 ft", "1 ft", "Precipitation"),
                       name = "") +
