...
VSM <- read.csv("G:/Shared drives/UGS_Groundwater/Projects/WRI/Tintic/Soil Moisture/RExports//T43NorthDeathDailyAveVSM.csv")
...
ggplot(daily, aes(x = Day)) +
  ...
  labs(title = "T43 North Death", x = "Date", y = "Average Soil Moisture m3/m3 VWC") +
  scale_color_discrete(breaks = c("Port_1", "Port_2", "Port_3", "Precipitation"), 
                       labels = c("1.5 ft", "4.5 ft", "10 ft", "Precipitation"),
                       name = "") +
