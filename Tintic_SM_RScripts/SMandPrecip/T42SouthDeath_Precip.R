...
VSM <- read.csv("G:/Shared drives/UGS_Groundwater/Projects/WRI/Tintic/Soil Moisture/RExports//T42SouthDeathDailyAveVSM.csv")
...
daily <- merge(Precip, VSM, by = "Day", all.x = TRUE)
...
ggplot(daily, aes(x = Day)) +
  ...
  labs(title = "T42 South Death", x = "Date", y = "Average Soil Moisture m3/m3 VWC") +
  scale_color_discrete(breaks = c("Port_1", "Port_2", "Port_3", "Precipitation"), 
                       labels = c("42 in", "25 in", "12 in", "Precipitation"),
                       name = "") +
